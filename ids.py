...
"""
Performs Depth-first Search on a graph and returns true if the goal was found.
...
"""
def perform_ids(root: Vertex, goal: Vertex, graph: Graph, canvas, max_depth) -> bool:
  for i in range( 1 , max_depth +1):
    if perform_dls(root, goal, graph, canvas, i) : return True
  return False 


def perform_dls(root: Vertex, goal: Vertex, graph: Graph, canvas, depth_limit) -> bool:
...
if __name__ == "__main__":
  window = tk.Tk()
  window.title("Depth-first Search UI demo")
  ...
  print(perform_ids(demo_root, demo_goal, demo_graph, canvas, 3))
